...
This file was last updated on: March 3rd, 2021
...
* [Installation](#Installation)  
    ...
    * [Unix](#Installation-Unix)  
        * [General setup](#Installation-Unix-Standard)  
...
SNMPTT can log to any of the following destinations: text log, syslog, Windows Event log or a SQL database. External programs can also be run to pass the translated trap to an email client, paging software, Nagios, Icinga etc.
...
SNMPTT can be downloaded from [http://www.sourceforge.net/projects/snmptt](http://www.sourceforge.net/projects/snmptt)

# <a name="Requirements"></a>Requirements

*   Perl 5.6.1 or higher.  SNMPTT is developed with 5.6.1 and 5.8.0, but newer versions will also work.
...
*   Optional: [Socket](http://search.cpan.org/search?module=Socket) module (included with most if not all distributions including ActivePerl). Required for DNS translations.
...
3. A startup script is included which can be used to start and stop **snmptt** on RedHat and other systems.  Copy the script to the **init.d** directory (renaming the file during the copy):  
...
6. SNMPTT will be started at the next reboot, or can be started immediately with:
...
7. To manually start without using the startup script, use:
...
9. Start snmptrapd using the command line to ensure there are no configuration errors:
...
10. Permanently change snmptrapd to use the -On option by modifying the startup script:  
    ...
    1. Systemd:  
        ...
        ```
        Environment=OPTIONS="-LsdOn"
        ```
  
  
    2. SysVinit:  
  
        Edit the **/etc/rc.d/init.d/snmptrapd** and add **"-On"** to **OPTIONS**:
...
1. Make sure snmptrapd has embedded Perl support enabled.  When compiling from source, use the '--enable-embedded-perl' configuration option.  Type:

    ```
    snmptrapd -H 2>&1 | grep perl
    ```
    
    It should give perl   PERLCODE if embedded Perl is enabled.  
...
2. Copy **snmptt.ini-nt** to **%SystemRoot%\\snmptt.ini** (c:\\winnt\\snmptt.ini) and edit the options inside the file.
...
The location of the ini file can be set on the command line using the **\-ini=** parameter.  See [Command Line Arguments](#Command-line-arguments).
...
> **enable\_unknown\_trap\_log**  
> **unknown\_trap\_log\_file  
> **
...
Translated traps can be sent to syslog.  The format of the entries will be similar to above without the date (as syslogd adds the date):
...
> **syslog\_system\_enable**  
> **syslog\_system\_facility**  
> **syslog\_system\_level**
...
> **mysql\_dbi\_host  
> ****mysql\_dbi\_port**  
> **mysql\_dbi\_database**  
> **mysql\_dbi\_table  
> ****mysql\_dbi\_table\_unknown**  
> **mysql\_dbi\_username**  
> **mysql\_dbi\_password**
...
If logging of unknown traps to a SQL table is required, create the **snmptt\_unknown** table using:
    ...
    DROP TABLE snmptt\_unknown;   
    CREATE TABLE snmptt\_unknown (   
    trapoid VARCHAR(100),  
    enterprise VARCHAR(100),   
    community VARCHAR(20),   
    hostname VARCHAR(100),   
...
> **postgresql\_dbi\_enable**  
> **postgresql\_dbi\_module**
> **postgresql\_dbi\_hostport\_enable**
> **postgresql\_dbi\_host**
> **postgresql\_dbi\_port**  
> **postgres****ql\_dbi\_database**  
> **postgres****ql\_dbi\_table**
> ****postgres****ql\_dbi\_table\_unknown**  
> **postgres****ql\_dbi\_username**  
> **postgres****ql\_dbi\_password**
...
If logging of statistics to a SQL table is required, create the snmptt\_statistics table using:  
    ...
    GRANT ALL ON snmptt\_statistics TO snmptt;  
    \q
...
> **dbd\_odbc\_enable = 1;**  
> **dbd\_odbc\_dsn = 'snmptt';**  
> **dbd\_odbc\_table = 'snmptt';  
> ****dbd\_odbc\_table\_unknown = 'snmptt';**  
> **dbd\_odbc\_username = 'snmptt';**  
> **dbd\_odbc\_password = 'password';**
...
If logging of statistics to a SQL table is required, create the snmptt\_statistics table using:  
    ...
    CREATE TABLE snmptt\_statistics (  
    stat\_time character(30) NULL,  
    total\_received BIGINT NULL,  
    total\_translated BIGINT NULL,  
    total\_ignored BIGINT NULL,  
    total\_unknown BIGINT NULL);
...
> **sql\_win32\_odbc\_enable = 1;**  
> **sql\_win32\_odbc\_dsn = 'snmptt';**  
> **sql\_win32\_odbc\_table = 'snmptt';**  
> **sql\_win32\_odbc\_username = 'snmptt';**  
> **sql\_win32\_odbc\_password = 'password';**
...
An external program can be launched when a trap is received.  The command line is defined in the configuration file.  For example, to send a page using QPAGE ([http://www.qpage.org](http://www.qpage.org)), the following command line could be used:
...
> > $v_n_  - Expand variable name of the variable-binding n (1-_n_)**(see Note 3)**  
...
> > 
> > For this feature to work, you must ensure Net-SNMP is configured correctly with all the required MIBS.  If the option is enabled, but the value can not be found, the integer value will be used.  If the MIB files are present, but translations do not occur, ensure Net-SNMP is correctly configured to process all the required mibs.  This is configured in the Net-SNMP **snmp.conf** file.  Alternatively, you can try setting the **mibs\_enviroment** variable in **snmptt.ini** to **ALL** (no quotes) to force all MIBS to be initialized at SNMPTT startup.
> > 
...
> Note (3):
> 
> > $v_n_, $+_n_ and $-_n_ variable names and variable type are translated into the text name by performing a lookup in the MIB file.  You must have the Net-SNMP Perl module installed for this to work and you must enable support for it by enabling **net\_snmp\_perl\_enable** in the snmptt.ini file.  If **net\_snmp\_perl\_enable** is not enabled, the $v_n_ variable will be replaced with the text 'variable_n_' where _n_ is the variable number (1+).
> > 
...
For freshness checking to work, **normal\_check\_interval** must be set to **1**, **valid check\_period** should be set to **24x7** and the following service definitions should be added.  The third column should not be included in the definition.  
...
To configure a heartbeat trap, start by creating a new service definition by following 'Creating the Nagios service entry' above, but use a new service\_description such as MyApp\_heartbeat.  Next, add / change the following service definitions.  The third column should not be included in the definition.  
...
When defining the service for receiving the SNMPTT translated trap, the service must be defined as volatile.  When a service is changed from an OK state to a non-OK state, contacts are notified.  Normally, a service in Nagios is not defined volatile which means if another service check is performed and the state is still non-OK then NO contacts are notified.  Because there is only one service entry for SNMP traps, we need to make sure we are contacted every time a trap comes in.
...
Note:  To simplify the configuration, you can instead apply the service too hosts using 'apply Service'.  See the [Icinga SNMPTT documentation](https://icinga.com/docs/icinga-2/snapshot/doc/07-agent-based-monitoring/#snmp-traps-and-passive-check-results) for an example.
...
Create an EXEC statement such as the following for each **EVENT** entry in your snmptt.conf file:  
    ...
    EXEC /usr/bin/curl -k -s -S -i -u _apiuser_:_password_ -H 'Accept: application/json' -X POST 'https://localhost:5665/v1/actions/process-check-result' -d '{ "type": "Service", "filter": "host.name==\\"$A\\" && service.name==\\"TRAP\\"", "exit\_status": 1, "plugin\_output": "xxxxxx", "check\_source": "$A", "pretty": true }'  
...
The variable substitution **$A** is used to pass the host name for **host.name** and **check\_source**, TRAP for **service.name** matches the service definition defined above, **exit\_status** of **1** represents a **WARNING**, and "xxxxxx" is the translated message from SNMPTT.  
...
To have the TRAP service automatically cleared every 24 hours, modify the service entry to enable active checks and define a check\_interval:  
...
* If a linkDown is received (node=x msg\_text=cisco linkdowni trap on interface x from SNMPTT), and then a linkUp is received within 20 seconds, it is considered a BOUNCE.  A new 'event' is created with the internal SEC event 'CISCO %1 INTERFACE %2 BOUNCE' is created which is passed to the other rules. 
...
Information on handling SNMP traps with [Hobbit](http://hobbitmon.sourceforge.net/) can be found at [http://cerebro.victoriacollege.edu/hobbit-trap.html](http://cerebro.victoriacollege.edu/hobbit-trap.html).